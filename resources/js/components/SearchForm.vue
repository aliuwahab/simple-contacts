<template>
    <div>
        <div v-if="focus" @click="focus = false" class="bg-black opacity-25 right-0 bottom-0 top-0 left-0 z-10"></div>
        ...
        <div class="relative z-10">
            ...
            <input class="w-64 mr-6 bg-gray-200 border border-gray-400 pl-8 pr-3 py-1 rounded-full text-sm focus:outline-none focus:border-blue-500 focus:shadow focus:bg-gray-100"
            ...
            <div v-if="focus" class="absolute bg-blue-900 text-white rounded-lg p-4 w-96 right-0 mr-6 mt-2 shadow z-20">
                <div v-if="results == 0"> No search results found for '{{ searchTerm}}'</div>
                <div v-for="result in results" @click="focus = false">
                    <router-link :to="result.links.self" class="block py-2">
                        <div class="flex items-center">
                            <UserNameCircle fullName="result.data.full_name"/>

                            <div class="pl-3">
                                <p class="">{{ result.data.full_name}}</p>
    ...
    export default {
        ...
        methods: {
            search: _.debounce(function (e) {
                ...
                axios.post('/api/search', {search_term: this.searchTerm})
                    .then(response => {
